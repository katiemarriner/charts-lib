...
/***/ function(module, exports, __webpack_require__) {
	...
	var StackedBar = __webpack_require__(4);
	...
	d3.json('src/data/stackedBar.json', function (data) {
		...
		var Stacked = new StackedBar('#app', data, {
			...
			rowKey: rowKey,
			keys: Object.keys(data[0]).filter(function (d) {
				return d != rowKey;
			})
			// dataPoints: [
			// 	{
...
/***/ function(module, exports, __webpack_require__) {
	...
	var StackedBar = function StackedBar(element, data, opts) {
	...
	StackedBar.prototype.createScales = function () {
		...
		this.series = d3.stack().keys(this.keys).offset(this.stackOffsetDiverging)(this.data);

		this.x = d3.scaleLinear().rangeRound([0, this.width]);
		this.y = d3.scaleBand().rangeRound([0, this.height]).paddingInner(0.05).align(0.1);
	...
	StackedBar.prototype.createAxes = function () {
		...
		this.xAxis = this.svg.append('g').attr('transform', 'translate(0,' + this.height + ')').call(d3.axisBottom(this.x));

		this.yAxis = this.svg.append('g').attr('transform', 'translate(' + m.left + ',0)').call(d3.axisLeft(this.y));
	};

	StackedBar.prototype.drawBars = function () {
		var _this = this;

		this.g = this.svg.append('g').selectAll('g').data(this.series).enter().append('g').attr('fill', function (d) {
			return _this.z(d.key);
		}).selectAll('rect').data(function (d) {
			return d;
		}).enter().append('rect').attr('height', this.y.bandwidth).attr('y', function (d) {
			return _this.y(d.data.month);
		}).attr('x', function (d) {
			return _this.x(d[0]);
		}) // changed to d[0] from d[1]
		.attr('width', function (d) {
			return _this.x(d[1]) - _this.x(d[0]);
		}); // reversed order of d[0] and d[1]
