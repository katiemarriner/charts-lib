...
/***/ function(module, exports, __webpack_require__) {
	...
	var MultiLine = __webpack_require__(4);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var MultiLine = function MultiLine(element, data, opts) {
	...
	MultiLine.prototype.createScales = function () {
		...
		this.x.domain([d3.min(this.data, function (c) {
		...
		this.y.domain([d3.min(this.data, function (c) {
			return d3.min(c.values, function (d) {
				return d['yKey'];
			});
		}), d3.max(this.data, function (c) {
	...
	MultiLine.prototype.createAxes = function () {
		this.g.append('g').attr('class', 'x-axis').attr('transform', 'translate(0,' + this.height + ')').call(this.axes.xPosition(this.x));

		this.g.append('g').attr('class', 'y-axis').call(this.axes.yPosition(this.y)).append('text').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '0.71em').attr('fill', '#000').text('Temperature, ºF');
	};
	...
	MultiLine.prototype.drawLines = function () {
		...
		this.lineGroups = this.g.selectAll('g.line-group').data(this.data).enter().append('g').attr('class', 'line-group');

		this.lineGroups.append('path').attr('class', 'line').attr('d', function (d) {
	...
	MultiLine.prototype.resize = function (newOpts) {
		...
		this.g.select('g.y-axis').call(this.axes.yPosition(this.y));
	...
	MultiLine.prototype.formatRecords = function (rawData, xParse, xKey, yKey) {
		...
		return lineName.map(function (d, di) {
			var xVals = Object.keys(rawData[d]);
			var yVals = rawData[d];
			return {
				key: d,
				values: xVals.map(function (e, ei) {
					return {
						xKey: _this2.parseTime ? _this2.parseTime(e) : e,
						yKey: yVals[e]
					};
				})
			};
