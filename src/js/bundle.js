...
/***/ function(module, exports, __webpack_require__) {
	...
	d3.json('src/data/line.json', function (data) {
		// var rowKey = 'month';
...
/***/ function(module, exports) {
	...
	var DataLoader = function DataLoader(opts) {
		var dataParams = opts.params;
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var swoopyArrow = __webpack_require__(7);
	...
	Annotation.prototype.draw = function (pointObj, index) {
		...
		oneAnnotation.drawLine = oneAnnotation.path.datum([[this.x(oneAnnotation.xKey) + 15, this.y(oneAnnotation.yKey) + 15], [this.x(oneAnnotation.xKey), this.y(oneAnnotation.yKey)]]).attr('d', oneAnnotation.swoopy);
...
/***/ function(module, exports) {
	...
	var swoopyArrow = function swoopyArrow() {
	  ...
	  function render(data) {
	    ...
	    */
	    var path = "M " + data[0][0] + "," + data[0][1] + " a " + r + "," + r + " 0 0," + (clockwise ? "1" : "0") + " " + (data[1][0] - data[0][0]) + "," + (data[1][1] - data[0][1]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	Line.prototype.init = function () {
		...
		this.svg = this.container.append('svg').attr('width', this.width - margin.left - margin.right).attr('height', this.height - margin.top - margin.bottom);
	...
	Line.prototype.assets = function () {
		...
		this.svg.append('svg:defs').append('svg:marker').attr('id', 'arrowhead').attr('viewBox', '0 -5 10 10').attr('refX', 5).attr('refY', 0).attr('markerWidth', this.markerWidth).attr('markerHeight', this.markerHeight).attr('orient', 'auto').append('svg:path').attr('fill', '#000').attr('stroke-width', 1).attr('stroke', '#000').attr('d', 'M0,-5L10,0L0,5');
	};
	...
	Line.prototype.createScales = function (newOpts) {
		...
		this.x = d3.scaleTime().rangeRound([0, this.width]).domain(d3.extent(this.data, function (d) {
			return d['xKey'];
		}));
		this.y = d3.scaleLinear().rangeRound([this.height, 0]).domain(d3.extent(this.data, function (d) {
			return d['yKey'];
		}));
	...
	Line.prototype.createAxes = function (newOpts) {
		...
		this.g.append('g').attr('transform', 'translate(0,' + this.height + ')').attr('class', 'x-axis').call(this.axes.xPosition(this.x));

		this.g.append('g').attr('class', 'y-axis').call(this.axes.yPosition(this.y)).append('text').attr('fill', '#000').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '0.71em').attr('text-anchor', 'end').text(this.yLabel);
	...
	Line.prototype.resize = function (newOpts) {
		...
		this.x.rangeRound([0, this.width]).domain(d3.extent(this.data, function (d) {
			return d['xKey'];
		}));

		this.y.rangeRound([this.height, 0]).domain(d3.extent(this.data, function (d) {
			return d['yKey'];
		}));

		this.g.select('g.x-axis').attr('transform', 'translate(0,' + this.height + ')').call(this.axes.xPosition(this.x));

		this.g.select('g.y-axis').call(this.axes.yPosition(this.y)).append('text').attr('fill', '#000').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '0.71em').attr('text-anchor', 'end').text(this.yLabel);
